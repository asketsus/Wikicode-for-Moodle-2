...
abstract class wikicode_markup_parser extends generic_parser {
    ...
    /**
     * function wikicode_parser_link_callback($link = "")
     *
     * Returns array('content' => "Inside the link", 'url' => "http://url.com/Wiki/Entry", 'new' => false).
     */
    private $linkgeneratorcallback = array('parser_utils', 'wikicode_parser_link_callback');
    ...
    private $tablegeneratorcallback = array('parser_utils', 'wikicode_parser_table_callback');

    /**
     * Get real path from relative path
     */
    private $realpathcallback = array('parser_utils', 'wikicode_parser_real_path');
    ...
    protected function generate_header($text, $level) {
        ...
        if (!$this->pretty_print && $level == 1) {
            $text .= parser_utils::h('a', $this->section_edit_text, array('href' => "edit.php?pageid={$this->wiki_page_id}&section=" . urlencode($text), 'class' => 'wikicode_edit_section'));
        }
    ...
    protected function format_image($src, $alt, $caption = "", $align = 'left') {
        ...
        return parser_utils::h('div', parser_utils::h('p', $caption) . '<img src="' . $src . '" alt="' . $alt . '" />', array('class' => 'wikicode_image_' . $align));
    }
    ...
    protected function format_link($text) {
        ...
        if (isset($link['new']) && $link['new']) {
            $options = array('class' => 'wikicode_newentry');
        } else {
            $options = array();
        }
