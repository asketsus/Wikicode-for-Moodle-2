...
class backup_wiki_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        // Define sources
        $wiki->set_source_table('wiki_code', array('id' => backup::VAR_ACTIVITYID));

        // All these source definitions only happen if we are including user info
        if ($userinfo) {
            $subwiki->set_source_sql('
                SELECT *
                  FROM {wiki_subwikis_code}
                 WHERE wikiid = ?', array(backup::VAR_PARENTID));

            $page->set_source_table('wiki_pages_code', array('subwikiid' => backup::VAR_PARENTID));

            $synonym->set_source_table('wiki_synonyms_code', array('subwikiid' => backup::VAR_PARENTID));

            $link->set_source_table('wiki_links_code', array('subwikiid' => backup::VAR_PARENTID));

            $version->set_source_table('wiki_versions_code', array('pageid' => backup::VAR_PARENTID));
            ...
            $tag->set_source_sql('SELECT t.id, t.name, t.rawname
                                   ...
                                   WHERE ti.itemtype = ?
                                     AND ti.itemid = ?', array(
                                         backup_helper::is_sqlparam('wiki_pages_code'),
                                         backup::VAR_PARENTID));
        }
        ...
        $wiki->annotate_files('mod_wikicode', 'intro', null); // This file area hasn't itemid
        $page->annotate_files('mod_wikicode', 'attachments', 'id'); // This file area hasn't itemid
